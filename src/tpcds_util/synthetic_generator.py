...
class SyntheticDataGenerator:
    ...
    def _initialize_lookups(self):
        ...
        self.brands = {
            'Electronics': ['TechCorp', 'DigitalPlus', 'InnovateTech', 'PowerElectronics', 'SmartDevices'],
            'Clothing': ['StyleMax', 'FashionForward', 'ComfortWear', 'UrbanStyle', 'ClassicFit'],
            'Home & Garden': ['HomeComfort', 'LivingSpace', 'CozyHome', 'ModernLiving', 'GardenPro'],
            'Sports': ['ActiveLife', 'SportsPro', 'FitnessFlex', 'OutdoorAdventure', 'TeamSpirit'],
            'Books': ['KnowledgePress', 'StoryBooks', 'LearnMore', 'ReadWell', 'BookCraft'],
            'Automotive': ['AutoPro', 'CarCare', 'DriveSmart', 'VehiclePlus', 'MotorMax']
        }
        ...
        self.seasonal_patterns = {
            'Electronics': {1: 0.8, 2: 0.9, 3: 1.0, 4: 1.0, 5: 1.0, 6: 1.1, 
                           7: 1.1, 8: 1.0, 9: 1.0, 10: 1.1, 11: 1.3, 12: 1.8},
            'Clothing': {1: 0.7, 2: 0.8, 3: 1.2, 4: 1.3, 5: 1.1, 6: 1.0, 
                        7: 0.9, 8: 1.0, 9: 1.2, 10: 1.1, 11: 1.4, 12: 1.6},
            'Home & Garden': {1: 0.8, 2: 0.9, 3: 1.3, 4: 1.4, 5: 1.3, 6: 1.1, 
                             7: 1.0, 8: 1.0, 9: 1.1, 10: 1.0, 11: 1.1, 12: 1.2},
            'Sports': {1: 1.2, 2: 1.1, 3: 1.2, 4: 1.3, 5: 1.4, 6: 1.3, 
                      7: 1.2, 8: 1.1, 9: 1.2, 10: 1.1, 11: 1.0, 12: 1.3}
        }
    ...
    def _generate_realistic_name(self, name_type: str = 'company') -> str:
        ...
        fallbacks = {
            'company': ['MegaCorp Inc', 'Global Solutions LLC', 'Premier Services', 'Advanced Systems'],
            'person_first': ['John', 'Mary', 'David', 'Sarah', 'Michael', 'Lisa', 'Robert', 'Jennifer'],
            'person_last': ['Smith', 'Johnson', 'Williams', 'Brown', 'Jones', 'Garcia', 'Miller', 'Davis'],
            'city': ['Springfield', 'Riverside', 'Franklin', 'Georgetown', 'Madison', 'Oak Hill']
        }
    ...
    def generate_date_dimension(self) -> List[Dict]:
        ...
        while current_date <= self.config.end_date:
            ...
            month_seq = (current_date.year - self.config.start_date.year) * 12 + current_date.month
            quarter_seq = (current_date.year - self.config.start_date.year) * 4 + ((current_date.month - 1) // 3) + 1
            
            # Determine if it's a holiday (simplified)
            is_holiday = 'Y' if (
                (current_date.month == 12 and current_date.day in [24, 25, 31]) or
                (current_date.month == 1 and current_date.day == 1) or
                (current_date.month == 7 and current_date.day == 4) or
                (current_date.month == 11 and 22 <= current_date.day <= 28 and current_date.weekday() == 3)  # Thanksgiving
            ) else 'N'
            ...
            date_record = {
                ...
                'd_fy_year': current_date.year if current_date.month >= 7 else current_date.year - 1,
                ...
                'd_first_dom': 1,
                'd_last_dom': (current_date.replace(month=current_date.month % 12 + 1, day=1) - timedelta(days=1)).day,
                'd_same_day_ly': (current_date - timedelta(days=365)).toordinal() if current_date >= self.config.start_date + timedelta(days=365) else None,
                'd_same_day_lq': (current_date - timedelta(days=90)).toordinal() if current_date >= self.config.start_date + timedelta(days=90) else None,
                'd_current_day': 'Y' if current_date == date.today() else 'N',
                'd_current_week': 'N',  # Simplified
                'd_current_month': 'Y' if current_date.year == date.today().year and current_date.month == date.today().month else 'N',
                'd_current_quarter': 'N',  # Simplified
                'd_current_year': 'Y' if current_date.year == date.today().year else 'N'
            }
    ...
    def generate_customer_demographics(self) -> List[Dict]:
        ...
        for i in range(10000):  # Standard demographic segments
            ...
            marital_status = random.choice(['S', 'M', 'W', 'D'])  # Single, Married, Widowed, Divorced
            education_levels = ['Primary', 'Secondary', 'College', 'Advanced Degree', 'Unknown']
            ...
            # Dependents
            dep_count = random.choices([0, 1, 2, 3, 4, 5], weights=[20, 25, 30, 15, 8, 2])[0]
            dep_employed = min(dep_count, random.randint(0, dep_count)) if dep_count > 0 else 0
            dep_college = min(dep_count, random.randint(0, dep_count // 2)) if dep_count > 0 else 0
        ...
        self.dimension_keys['customer_demographics'] = [d['cd_demo_sk'] for d in demographics]
        return demographics
    ...
    def generate_customer_addresses(self) -> List[Dict]:
        ...
        for region, states in self.regions.items():
            addresses_per_region = self.config.num_customers // 4  # Divide among 4 regions
            ...
            for i in range(addresses_per_region):
                ...
                address_record = {
                    ...
                    'ca_street_type': random.choice(['St', 'Ave', 'Blvd', 'Dr', 'Ln', 'Rd', 'Way']),
                    'ca_suite_number': f'Suite {random.randint(1, 200)}' if random.random() < 0.3 else None,
                    ...
                    'ca_location_type': random.choice(['apartment', 'condo', 'single family', 'unknown'])
                }
                
                addresses.append(address_record)
        
        self.generated_data['customer_address'] = addresses
        self.dimension_keys['customer_address'] = [a['ca_address_sk'] for a in addresses]
        return addresses
    ...
    def generate_warehouses(self) -> List[Dict]:
        ...
        for i in range(self.config.num_warehouses):
            ...
            warehouse_record = {
                ...
                'w_suite_number': f'Bldg {random.randint(1, 20)}' if random.random() < 0.4 else None,
    ...
    def generate_stores(self) -> List[Dict]:
        ...
        for i in range(self.config.num_stores):
            ...
            store_record = {
                ...
                's_division_name': random.choice(['North Division', 'South Division', 'Central Division']),
                ...
                's_suite_number': f'Suite {random.randint(1, 50)}' if random.random() < 0.2 else None,
    ...
    def generate_items(self) -> List[Dict]:
        ...
        for i in range(self.config.num_items):
            ...
            wholesale_cost = round(current_price * random.uniform(0.4, 0.7), 2)  # 40-70% of retail
            
            # Product attributes
            colors = ['red', 'blue', 'green', 'black', 'white', 'gray', 'brown', 'yellow', 'purple', 'orange']
            ...
            item_record = {
                ...
                'i_container': random.choice(['Box', 'Bag', 'Case', 'Carton', 'Package']),
    ...
    def generate_customers(self) -> List[Dict]:
        ...
        for i in range(self.config.num_customers):
            ...
            customer_record = {
                ...
                'c_first_shipto_date_sk': random.choice(self.dimension_keys['date_dim']),
    ...
    def generate_additional_dimensions(self) -> Dict[str, List[Dict]]:
        ...
        for hour in range(24):
            for minute in range(0, 60, 15):  # Every 15 minutes
                ...
                meal_time = 'Breakfast' if 6 <= hour < 10 else 'Lunch' if 11 <= hour < 14 else 'Dinner' if 17 <= hour < 20 else 'Off Hours'
        ...
        # Return Reasons
        reasons = [
            'Defective item', 'Wrong item shipped', 'Item not as described', 
            'Changed mind', 'Found better price', 'Gift return',
            'Size too small', 'Size too large', 'Color not as expected',
            'Damaged in shipping', 'Arrived too late', 'Duplicate order'
        ]
        ...
        self.dimension_keys['income_band'] = [ib['ib_income_band_sk'] for ib in income_bands]
        ...
        for i in range(7200):  # Standard TPC-DS household demographic combinations
            ...
            buy_potential_levels = ['Unknown', '0-500', '501-1000', '1001-5000', '5001-10000', '10001+']
            dep_counts = [0, 1, 2, 3, 4, 5, 6]
            vehicle_counts = [0, 1, 2, 3, 4]
            
            household_demos.append({
                'hd_demo_sk': hdemo_sk,
                'hd_income_band_sk': random.choice(self.dimension_keys['income_band']),
        ...
        self.dimension_keys['household_demographics'] = [hd['hd_demo_sk'] for hd in household_demos]
    ...
    def generate_call_centers(self) -> List[Dict]:
        ...
        self.dimension_keys['call_center'] = [cc['cc_call_center_sk'] for cc in call_centers]
        return call_centers
    ...
    def generate_inventory(self) -> List[Dict]:
        ...
        sample_dates = random.sample(self.dimension_keys['date_dim'], min(10, len(self.dimension_keys['date_dim'])))
        sample_items = random.sample(self.dimension_keys['item'], min(500, len(self.dimension_keys['item'])))
        ...
        for date_sk in sample_dates:
            for warehouse_sk in self.dimension_keys['warehouse']:
                for item_sk in sample_items:
                    ...
                    item_record = next(i for i in self.generated_data['item'] if i['i_item_sk'] == item_sk)
                    base_qty = {'Electronics': 25, 'Clothing': 100, 'Books': 50, 
                               'Sports': 30, 'Automotive': 15, 'Home & Garden': 40}.get(item_record['i_category'], 25)
        ...
        for category in self.product_categories.keys():
            for subcategory in self.product_categories[category]:
                ...
                web_pages.append({
                    ...
                    'wp_creation_date_sk': random.choice(self.dimension_keys['date_dim']),
                    'wp_access_date_sk': random.choice(self.dimension_keys['date_dim']),
    ...
    def generate_catalog_pages(self) -> List[Dict]:
        ...
        for category in self.product_categories.keys():
            for i in range(2):  # 2 pages per category (seasonal catalogs)
                ...
                catalog_pages.append({
                    ...
                    'cp_start_date_sk': random.choice(self.dimension_keys['date_dim']),
                    'cp_end_date_sk': random.choice(self.dimension_keys['date_dim']),
        ...
        self.dimension_keys['catalog_page'] = [cp['cp_catalog_page_sk'] for cp in catalog_pages]
    ...
    def generate_web_sales(self) -> List[Dict]:
        ...
        num_transactions = min(2000, len(self.dimension_keys['customer']) // 5)  # Smaller for faster loading
        ...
        for i in range(num_transactions):
            ...
            web_sales.append({
                ...
                'ws_ship_date_sk': date_sk + random.randint(1, 7),  # Ships 1-7 days later
                'ws_item_sk': item_sk,
                'ws_bill_customer_sk': customer_sk,
                'ws_bill_cdemo_sk': random.choice(self.dimension_keys['customer_demographics']),
                'ws_bill_hdemo_sk': random.choice(self.dimension_keys['household_demographics']),
                'ws_bill_addr_sk': random.choice(self.dimension_keys['customer_address']),
                'ws_ship_customer_sk': customer_sk,  # Same as bill for simplicity
                'ws_ship_cdemo_sk': random.choice(self.dimension_keys['customer_demographics']),
                'ws_ship_hdemo_sk': random.choice(self.dimension_keys['household_demographics']),
                'ws_ship_addr_sk': random.choice(self.dimension_keys['customer_address']),
                ...
                'ws_promo_sk': random.choice(self.dimension_keys['promotion']) if random.random() < 0.2 else None,
                'ws_order_number': ws_sk,
                'ws_quantity': quantity,
                'ws_wholesale_cost': wholesale_cost,
                'ws_list_price': list_price,
                'ws_sales_price': sales_price,
                'ws_ext_discount_amt': round((list_price - sales_price) * quantity, 2),
    ...
    def generate_catalog_sales(self) -> List[Dict]:
        ...
        num_transactions = min(1500, len(self.dimension_keys['customer']) // 10)  # Smaller for faster loading
        ...
        for i in range(num_transactions):
            ...
            catalog_sales.append({
                ...
                'cs_ship_date_sk': date_sk + random.randint(3, 14),  # Ships 3-14 days later
                'cs_bill_customer_sk': customer_sk,
                'cs_bill_cdemo_sk': random.choice(self.dimension_keys['customer_demographics']),
                'cs_bill_hdemo_sk': random.choice(self.dimension_keys['household_demographics']),
                'cs_bill_addr_sk': random.choice(self.dimension_keys['customer_address']),
                'cs_ship_customer_sk': customer_sk,
                'cs_ship_cdemo_sk': random.choice(self.dimension_keys['customer_demographics']),
                'cs_ship_hdemo_sk': random.choice(self.dimension_keys['household_demographics']),
                'cs_ship_addr_sk': random.choice(self.dimension_keys['customer_address']),
                'cs_call_center_sk': random.choice(self.dimension_keys['call_center']),
                ...
                'cs_item_sk': item_sk,
                'cs_promo_sk': random.choice(self.dimension_keys['promotion']) if random.random() < 0.25 else None,
                'cs_order_number': cs_sk,
                'cs_quantity': quantity,
                'cs_wholesale_cost': wholesale_cost,
                'cs_list_price': list_price,
                'cs_sales_price': sales_price,
                'cs_ext_discount_amt': round((list_price - sales_price) * quantity, 2),
                'cs_ext_sales_price': ext_sales_price,
                'cs_ext_wholesale_cost': ext_wholesale_cost,
                'cs_ext_list_price': ext_list_price,
                'cs_ext_tax': ext_tax,
                'cs_coupon_amt': round(random.uniform(0, 10), 2) if random.random() < 0.15 else 0,
    ...
    def generate_store_returns(self) -> List[Dict]:
        ...
        sales_to_return = random.sample(self.generated_data['store_sales'], min(num_returns, len(self.generated_data['store_sales'])))
        ...
        for i, sale in enumerate(sales_to_return):
            ...
            store_returns.append({
                ...
                'sr_return_amt_inc_tax': round((sale['ss_ext_sales_price'] + sale['ss_ext_tax']) * ratio, 2),
                'sr_fee': round(random.uniform(0, 5), 2),
                'sr_return_ship_cost': 0,  # Store returns don't have shipping
                'sr_refunded_cash': round(sale['ss_ext_sales_price'] * ratio * 0.8, 2),  # 80% cash refund
                'sr_reversed_charge': round(sale['ss_ext_sales_price'] * ratio * 0.2, 2),  # 20% credit
                'sr_store_credit': round(sale['ss_ext_sales_price'] * ratio * 0.1, 2),  # 10% store credit
    ...
    def generate_web_returns(self) -> List[Dict]:
        ...
        sales_to_return = random.sample(self.generated_data['web_sales'], min(num_returns, len(self.generated_data['web_sales'])))
        
        for i, sale in enumerate(sales_to_return):
            wr_sk = i + 1
            
            return_date_sk = sale['ws_sold_date_sk'] + random.randint(1, 45)  # Longer return window online
            reason_sk = random.choice(self.dimension_keys['reason'])
            ...
            web_returns.append({
                ...
                'wr_returned_time_sk': random.choice(self.dimension_keys['time_dim']),
                ...
                'wr_return_amt_inc_tax': round((sale['ws_ext_sales_price'] + sale['ws_ext_tax']) * ratio, 2),
                'wr_fee': round(random.uniform(2, 8), 2),  # Return processing fee
                'wr_return_ship_cost': round(sale['ws_ext_ship_cost'] * ratio, 2),
                'wr_refunded_cash': round(sale['ws_ext_sales_price'] * ratio * 0.9, 2),
                'wr_reversed_charge': round(sale['ws_ext_sales_price'] * ratio * 0.1, 2),
                'wr_account_credit': round(sale['ws_ext_sales_price'] * ratio * 0.05, 2),
    ...
    def generate_catalog_returns(self) -> List[Dict]:
        ...
        sales_to_return = random.sample(self.generated_data['catalog_sales'], min(num_returns, len(self.generated_data['catalog_sales'])))
        
        for i, sale in enumerate(sales_to_return):
            cr_sk = i + 1
            
            return_date_sk = sale['cs_sold_date_sk'] + random.randint(1, 60)  # Longer return window
            reason_sk = random.choice(self.dimension_keys['reason'])
            ...
            catalog_returns.append({
                ...
                'cr_return_amt_inc_tax': round((sale['cs_ext_sales_price'] + sale['cs_ext_tax']) * ratio, 2),
                'cr_fee': round(random.uniform(3, 10), 2),
                'cr_return_ship_cost': round(sale['cs_ext_ship_cost'] * ratio, 2),
                'cr_refunded_cash': round(sale['cs_ext_sales_price'] * ratio * 0.85, 2),
                'cr_reversed_charge': round(sale['cs_ext_sales_price'] * ratio * 0.15, 2),
                'cr_store_credit': round(sale['cs_ext_sales_price'] * ratio * 0.1, 2),
    ...
    def generate_store_sales(self) -> List[Dict]:
        ...
        sample_dates = [d for i, d in enumerate(self.generated_data['date_dim']) if i % 7 == 0]
        ...
        for date_record in sample_dates:
            ...
            for store_record in self.generated_data['store']:
                ...
                store_multiplier = store_record['s_floor_space'] / 50000  # Normalize to average size
                ...
                for _ in range(store_transactions):
                    ...
                    discount_pct = random.uniform(0, 0.3) if random.random() < 0.4 else 0
                    sales_price = round(list_price * (1 - discount_pct), 2)
                    ...
                    ext_wholesale_cost = round(wholesale_cost * quantity, 2)
                    ext_list_price = round(list_price * quantity, 2)
                    ext_tax = round(ext_sales_price * store_record['s_tax_precentage'], 2)
                    coupon_amt = round(random.uniform(0, 5), 2) if random.random() < 0.1 else 0
                    ...
                    sale_record = {
                        ...
                        'ss_cdemo_sk': random.choice(self.dimension_keys['customer_demographics']),
                        'ss_hdemo_sk': random.choice(self.dimension_keys['household_demographics']),
                        'ss_addr_sk': random.choice(self.dimension_keys['customer_address']),
                        'ss_store_sk': store_sk,
                        'ss_promo_sk': random.randint(1, 300) if random.random() < 0.3 else None,
                        'ss_ticket_number': len(store_sales) + 1,
                        'ss_quantity': quantity,
                        'ss_wholesale_cost': wholesale_cost,
                        'ss_list_price': list_price,
                        'ss_sales_price': sales_price,
                        'ss_ext_discount_amt': round((list_price - sales_price) * quantity, 2),
                    ...
                    if len(store_sales) >= 100000:  # 100K records max for faster loading
                        break
    ...
    def _get_gmt_offset(self, state: str) -> float:
        ...
        timezone_map = {
            # Pacific: -8
            'CA': -8.0, 'WA': -8.0, 'OR': -8.0, 'NV': -8.0,
            # Mountain: -7  
            'AZ': -7.0, 'UT': -7.0, 'CO': -7.0, 'ID': -7.0, 'MT': -7.0, 'WY': -7.0,
            # Central: -6
            'TX': -6.0, 'OK': -6.0, 'AR': -6.0, 'LA': -6.0, 'MS': -6.0, 'AL': -6.0,
            'TN': -6.0, 'IL': -6.0, 'WI': -6.0, 'MN': -6.0, 'IA': -6.0, 'MO': -6.0,
            'ND': -6.0, 'SD': -6.0, 'NE': -6.0, 'KS': -6.0,
            # Eastern: -5
            'FL': -5.0, 'GA': -5.0, 'NC': -5.0, 'SC': -5.0, 'VA': -5.0,
            'NY': -5.0, 'NJ': -5.0, 'PA': -5.0, 'CT': -5.0, 'MA': -5.0,
            'ME': -5.0, 'VT': -5.0, 'NH': -5.0, 'RI': -5.0, 'IN': -5.0, 'OH': -5.0, 'MI': -5.0
        }
    ...
    def write_to_files(self, output_dir: str):
        ...
        with Progress(
            ...
            task = progress.add_task("Generating synthetic data...", total=len(tables_to_generate))
            ...
            console.print(f"✅ Generated {total_files} synthetic data files", style="green")
    
    def _write_table_to_file(self, output_path: Path, table_name: str, data: List[Dict]):
...
def create_synthetic_data(scale: int = 1, output_dir: str = "./synthetic_data") -> bool:
    ...
    if not HAS_FAKER:
        console.print("⚠️  Faker library not found. Installing for better synthetic data...", style="yellow")
        try:
            import subprocess
            ...
            console.print(f"⚠️  Could not install Faker: {e}. Using basic synthetic data.", style="yellow")
    
    try:
        # Scale the configuration based on scale factor
        ...
        console.print(f"🎉 Synthetic TPC-DS data generated successfully!", style="bold green")
        console.print(f"📁 Files written to: {Path(output_dir).absolute()}", style="cyan")
        console.print("📋 This data is synthetic and license-free for enterprise use", style="green")
